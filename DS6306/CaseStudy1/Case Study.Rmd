---
title: "10.22.20 Budweiser Analysis"
author: "Feby Thomas"
date: "10/22/2020"
...
The analysis and code below is based off of the beer and brewery information provided by executive teams at Budweiser.  The findings from the SMU Consulting Company, were prepared and are represented by data scientists: Zach Zaiken, Feby Thomas and Rinku Lichti.  In the Code that follows, the team will be providing data inferences and key takeaways for each of the Budweiser Team's questions and inquiries.  The presentation is divided up into the sections below.
...
The code below creates a bar chart showing the brewery density by each state's population.  The States are sorted in descending order based on State Population, similary the states are color coded to show which states have the highest populations.  The bars in the chart show the brewery density while the labels show the population of each state.  The reccommendation would be for Budweiser to amp up opening up breweries in the Group 1 & Group 2 State Population Ranking groups.  States like CA, TX, FL have potential for many more breweries to be oponed based on state population and are nowhere near as dense as states like CO, OR or VT.
...
breweries_count <- breweries_count%>%unite("comb", c("State", "Population"), sep = ":", remove = FALSE)

breweries_count%>%ggplot(aes(y=State,x=count/ Population,fill=State_Population_Ranking))+
  
  geom_col(position = "dodge",col="black")+xlab("Number of Breweries per Million Population")+
  
  theme(axis.text.y = element_text(angle = 0))+
  
  ggtitle("Brewery Density by State Population")+
